...
bool SqlAccountsRepository::upsertAccount(const domain::Account &account) {
    ...
    query.prepare("INSERT INTO accounts(account_id, server_id, user_id, username, active_device_id, "
...
bool SqlConversationsRepository::upsertConversation(const domain::Conversation &conversation) {
    QSqlQuery query(m_db);
    query.prepare("INSERT INTO conversations(conversation_id, server_id, type, title_ciphertext, "
...
bool SqlMessagesRepository::upsertMessage(const domain::Message &message) {
    QSqlQuery query(m_db);
    query.prepare("INSERT INTO messages(message_id, conversation_id, sender_user_id, sender_device_id, "
...
QVector<domain::Message> SqlMessagesRepository::listConversationMessages(const QString &conversationId,
    ...
    QSqlQuery query(m_db);
    query.prepare("SELECT message_id, conversation_id, sender_user_id, sender_device_id, server_seq, "
...
bool SqlDevicesRepository::upsertDevice(const domain::Device &device) {
    QSqlQuery query(m_db);
    query.prepare("INSERT INTO devices(device_id, user_id, server_id, label, identity_sign_pub, identity_dh_pub, "
...
QVector<domain::Device> SqlDevicesRepository::listDevicesByUser(const QString &userId) const {
    ...
    QSqlQuery query(m_db);
    query.prepare("SELECT device_id, user_id, server_id, label, identity_sign_pub, identity_dh_pub, "
...
bool SqlSyncRepository::upsertRecord(const domain::SyncRecord &record) {
    QSqlQuery query(m_db);
    query.prepare("INSERT INTO sync_records(collection, record_id, version, ciphertext, updated_at_ms, tombstone) "
                  "VALUES(?, ?, ?, ?, ?, ?) "
...
QVector<domain::SyncRecord> SqlSyncRepository::listRecords(const QString &collection) const {
    ...
    QSqlQuery query(m_db);
    query.prepare("SELECT collection, record_id, version, ciphertext, updated_at_ms, tombstone "
...
bool SqlJobsRepository::enqueueJob(const QString &type, QByteArray payloadCiphertext, qint64 nextAttemptMs) {
    QSqlQuery query(m_db);
    query.prepare("INSERT INTO jobs(type, payload_ciphertext, state, attempt_count, next_attempt_ms) "
                  "VALUES(?, ?, 'queued', 0, ?)");
...
QVector<JobRecord> SqlJobsRepository::dueJobs(qint64 nowMs) const {
    ...
    QSqlQuery query(m_db);
    query.prepare("SELECT job_id, type, payload_ciphertext, state, attempt_count, next_attempt_ms "
...
bool SqlSearchRepository::insertToken(const QString &conversationId,
                                      ...
                                      int positionHint) {
    QSqlQuery query(m_db);
    query.prepare("INSERT INTO search_tokens(conversation_id, message_id, token_digest, position_hint) "
                  "VALUES(?, ?, ?, ?)");
    query.addBindValue(conversationId);
...
bool SqlTrustRepository::upsertTrust(const domain::DeviceFingerprint &trust) {
    QSqlQuery query(m_db);
    query.prepare("INSERT INTO trusted_devices(device_id, fingerprint, trust_level, verified_at_ms, note_ciphertext) "
                  "VALUES(?, ?, ?, ?, ?) "
...
std::optional<domain::DeviceFingerprint> SqlTrustRepository::findTrust(const QString &deviceId) const {
    ...
    trust.trustLevel =
        domain::trustLevelFromString(query.value(2).toString()).value_or(domain::TrustLevel::Unknown);
