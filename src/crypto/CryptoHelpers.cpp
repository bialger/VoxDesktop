...
QByteArray CryptoHelpers::hmacSha256(QByteArrayView key, QByteArrayView data) {
  ...
  crypto_auth_hmacsha256_init(
      &state, reinterpret_cast<const unsigned char *>(key.data()), static_cast<size_t>(key.size()));
  crypto_auth_hmacsha256_update(
      &state, reinterpret_cast<const unsigned char *>(data.data()), static_cast<unsigned long long>(data.size()));
...
bool CryptoHelpers::ed25519Verify(QByteArrayView pub, QByteArrayView msg, QByteArrayView sig) {
  if (!ensureSodiumInitialized() || pub.size() != crypto_sign_PUBLICKEYBYTES || sig.size() != crypto_sign_BYTES) {
    return false;
  }
...
QByteArray CryptoHelpers::argon2idKdf(
    QByteArrayView password, QByteArrayView salt, quint64 opslimit, size_t memlimit, size_t outLen) {
  if (!ensureSodiumInitialized() || outLen == 0 || salt.size() != crypto_pwhash_SALTBYTES || password.empty()) {
    return {};
  }
