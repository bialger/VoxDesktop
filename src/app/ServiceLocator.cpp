...
bool ServiceLocator::initialize(const QString &appDataPath,
    ...
    m_conversationService = std::make_unique<services::ConversationService>(*m_conversationsApi,
                                                                            *m_directoryApi,
                                                                            *m_conversationsRepository);
    m_messageSendService = std::make_unique<services::MessageSendService>(
        *m_conversationsApi, *m_messagesRepository, *m_jobsRepository);
    m_attachmentService =
        std::make_unique<services::AttachmentService>(*m_attachmentsApi, *m_fileStore);
