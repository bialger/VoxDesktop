...
ApiResult<AttachmentUploadInitResponse>
AttachmentsApi::uploadInit(const AttachmentUploadInitRequest &request) const {
    const auto response =
        m_network.postJson(ApiPaths::kAttachmentsUploadInit, JsonCodec::toBytes(toJson(request)));
...
VoidResult AttachmentsApi::uploadChunk(const QString &attachmentId,
    ...
    const auto response =
        m_network.putBytes(ApiPaths::kAttachmentsPrefix + attachmentId + "/chunk",
