...
ConversationsApi::createConversation(const ConversationCreateRequest &request) const {
...
VoidResult ConversationsApi::removeMember(const QString &conversationId, const QString &userId) const {
    return parseVoid(m_network.deleteRequest(ApiPaths::kConversations + "/" + conversationId + "/members/" +
                                             userId));
}
...
VoidResult ConversationsApi::unsubscribe(const QString &conversationId) const {
    return parseVoid(m_network.postJson(ApiPaths::kConversations + "/" + conversationId + "/unsubscribe",
                                        JsonCodec::toBytes({})));
}
...
ApiResult<EnvelopeBatchResponse> ConversationsApi::conversationEnvelopes(const QString &conversationId,
    ...
    const auto response =
        m_network.get(ApiPaths::kConversations + "/" + conversationId + "/envelopes", query);
