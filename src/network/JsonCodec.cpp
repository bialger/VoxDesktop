...
std::optional<QString> JsonCodec::requiredString(const QJsonObject &obj, const QString &key, QString *error) {
...
std::optional<qint64> JsonCodec::requiredInteger(const QJsonObject &obj, const QString &key, QString *error) {
  const auto value = obj.value(key);
  if (!value.isDouble()) {
    if (error != nullptr) {
