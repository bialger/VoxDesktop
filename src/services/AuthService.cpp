...
AuthService::AuthService(network::AuthApi &authApi,
                         ...
                         QString deviceId)
    : m_authApi(authApi),
      m_network(networkAccess),
      m_accountsRepository(accountsRepository),
      m_vault(vaultService),
      m_identity(identityKeyStore),
      m_prekeys(prekeyManager),
...
bool AuthService::restoreSession() {
    ...
    const auto refreshed = m_authApi.refresh(
        network::RefreshRequest{QString::fromUtf8(*refreshToken), account->activeDeviceId});
...
bool AuthService::persistSession(const QString &username, const network::AuthSessionResponse &response) {
    ...
    m_context = AuthContext{response.userId,
                            username,
                            m_deviceId,
