...
int Application::run(int argc, char **argv) {
    ...
    QObject::connect(realtime,
                     &network::RealtimeSocket::membershipChanged,
                     &window,
                     [&, services](const QString &, int) {
    ...
    QObject::connect(&window, &ui::shell::MainWindow::serverBaseUrlReady, [&, services, state](const QString &baseUrl) {
        ...
        QObject::connect(&window,
                         &ui::shell::MainWindow::conversationSelected,
                         [&, services, state](const QString &conversationId) {
                         ...
                         [&, services, state](const QString &username, const QString &passwordDerived) {
                             if (!services->authService()->registerUser(username, passwordDerived)) {
                                 QMessageBox::warning(&window,
                                                      "Registration failed",
                                                      "Could not register account with current server");
                                 return;
                             }
        ...
        QObject::connect(&window, &ui::shell::MainWindow::sendMessageSubmitted, [&, services, state](const QString &text) {
            ...
            const QString myRole =
                (convDto.ok && convDto.data.has_value()) ? convDto.data->myRole.toLower() : QString{};
            ...
            const bool ok = (sender != nullptr) &&
                            sender->sendMessage(state->selectedConversationId, auth->deviceId, text, ciphertext);
            if (!ok) {
    ...
    QObject::connect(&window,
                     ...
                     [&, services, state](const QString &conversationId) {
                         ...
                         if (summary.ok && summary.data.has_value()) {
                             ...
                             const auto auth = services->authService()->context();
                             const QString myUserId = auth.has_value() ? auth->userId : QString{};
                             state->selectedConversationIsSelfDm =
                                 (summary.data->type == 0 && summary.data->peerUserId.has_value() &&
                             ...
                             for (const auto &env : batch.data->envelopes) {
                                 ...
                                 if (plaintext.has_value()) {
                                     ...
                                     msg.plaintextCacheCiphertext =
                                         QString("[Encrypted envelope %1]").arg(env.envelopeId.right(6)).toUtf8();
                                 }
                                 ...
                                 if (msg.isOutgoing) {
                                     ...
                                     if (directory != nullptr && !senderUserId.isEmpty()) {
                                         ...
                                         if (resolved.ok && resolved.data.has_value() &&
                                             !resolved.data->username.isEmpty()) {
                                             display = resolved.data->username;
                                         }
                                     }
                                     msg.senderUserId = display;
    ...
    QObject::connect(&window,
                     &ui::shell::MainWindow::createGroupRequested,
                     [&, services, state](const QStringList &usernames) {
                         if (!servicesInitialized) {
                             QMessageBox::information(&window, "Not connected", "Connect to a server first.");
                             return;
                         }
                         ...
                         if (state->refreshConversations) {
                             ...
                             QTimer::singleShot(1500, &window, [state]() {
                                 if (state->refreshConversations) state->refreshConversations();
                             });
                         }
                     });

    QObject::connect(&window,
                     &ui::shell::MainWindow::createChannelRequested,
                     [&, services, state](const QStringList &adminUsernames) {
                         if (!servicesInitialized) {
                             QMessageBox::information(&window, "Not connected", "Connect to a server first.");
                             return;
                         }
    ...
    QObject::connect(&window,
                     &ui::shell::MainWindow::subscribeChannelRequested,
                     [&, services, state](const QString &conversationId) {
                         if (!servicesInitialized) {
                             QMessageBox::information(&window, "Not connected", "Connect to a server first.");
                             return;
